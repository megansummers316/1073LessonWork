...
form.addEventListener('submit', function(event) {
    console.log("form submitted at: " + event.timeStamp);
    const realTime = new Date();
    console.log("clicked at: " + realTime.toLocaleTimeString());
    if (email.value === '') {
        event.preventDefault();//stops form from submitting if email is empty
        console.log(event.type); //returns 'submit'
        console.log(event.target); //returns which event triggered it - 'form'
        console.log(event.timeStamp); //returns milliseconds after page has loaded
