...
const person = {
    ...
    age: 24,
    gender: "Female",
    interests: ["crocheting", "reading", "gaming"],
    greeting() {
        //alert(`Hi, I am ${person.name.first} ${person.name.last}, aged ${person.age} years old and I like ${person.interests[0]}.`)
        let greet = `Hi, I am ${this.name.first} ${this.name.last}, aged ${this.age} years old and I like ${this.interests[0]}.`; //better to use 'this' keyword instead of const name
        return greet; 
    }
};
...
person.eyes = "brown"; //you literally just add it like its a new variable without declaration
...
person.goodbye = function() {
    let goodbye = `${person.name.first} says goodbye!.`;
    return goodbye;
}
...
output.textContent = person1.hello();
